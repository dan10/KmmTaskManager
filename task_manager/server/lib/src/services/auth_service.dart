...
abstract class AuthService {
  Future<User> register(String name, String email, String password, {bool isSocialLogin = false});
...
class AuthServiceImpl implements AuthService {
  ...
  AuthServiceImpl(this._authRepository, this._jwtService, this._appConfig); // Update constructor

  @override
  Future<User> register(String name, String email, String password, {bool isSocialLogin = false}) async {
    ...
    if (existingUser != null) {
      ...
      if(isSocialLogin) {
        print('User $email already exists. Returning existing user for social login.');
    ...
    if (isSocialLogin) {
      ...
      passwordHashValue = "social_login_user_placeholder_hash_${email}_${DateTime.now().microsecondsSinceEpoch}";
    ...
    // Create new user
    final user = User(
      id: DateTime.now().millisecondsSinceEpoch.toString(), // Placeholder ID generation
      name: name,
      email: email,
  ...
  Future<User> login(String email, String password) async {
    ...
    if (user == null) {
      throw AuthenticationException(message: 'Invalid email or password.'); // Generic message
    }
  ...
  String _hashPassword(String password) {
    // This method is now only used by the old logic if not removed,
    // but the main logic uses direct hashing.
    // For completeness, if it were to be used, it should also hash.
    // However, since hashing is done directly in register/login, this can be removed or updated.
    // For now, let's update it to reflect the hashing strategy.
  ...
  Future<User> googleLogin(GoogleLoginRequestDto request) async {
    ...
    final uri = Uri.parse('https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=$idToken');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (tokenInfo['aud'] != expectedAudience) {
          throw AuthenticationException(message: 'Google ID token audience mismatch.');
        }

        // 2. Verify Issuer (iss)
        if (tokenInfo['iss'] != 'accounts.google.com' && tokenInfo['iss'] != 'https://accounts.google.com') {
          throw AuthenticationException(message: 'Invalid Google ID token issuer.');
        }

        // 3. Verify Expiry (exp)
        final expiry = tokenInfo['exp'] as int?;
        if (expiry == null || DateTime.fromMillisecondsSinceEpoch(expiry * 1000).isBefore(DateTime.now())) {
          throw AuthenticationException(message: 'Google ID token has expired.');
        }

        // If all checks pass, extract user information
        final email = tokenInfo['email'] as String?;
        if (email == null || email.isEmpty) {
          throw AuthenticationException(message: 'Email not found in Google ID token.');
        }
        
        final isEmailVerified = tokenInfo['email_verified'] == 'true' || tokenInfo['email_verified'] == true;
        if (!isEmailVerified) {
        ...
        throw AuthenticationException(message: 'Failed to verify Google ID token. Status: ${response.statusCode}, Body: ${response.body}');
      }
    } catch (e) {
      if (e is AuthenticationException) rethrow; // Re-throw our specific exceptions
      print('Error during Google ID token verification: $e');
      throw AuthenticationException(message: 'Error during Google ID token verification: ${e.toString()}');
    }
  }
}
