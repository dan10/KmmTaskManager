...
fun Application.configureDatabase() {
    ...
    val db = R2dbcDatabase.connect {
        val optionsBuilder = ConnectionFactoryOptions.parse(databaseConfig.url).mutate()
        if (databaseConfig.user.isNotBlank()) {
            optionsBuilder.option(ConnectionFactoryOptions.USER, databaseConfig.user)
        }
        if (databaseConfig.password.isNotBlank()) {
            optionsBuilder.option(ConnectionFactoryOptions.PASSWORD, databaseConfig.password)
        }
        optionsBuilder.build()
        ConnectionFactories.get(optionsBuilder.build())
    }
    ...
    runBlocking {
        suspendTransaction(db = db) {
            ...
            R2dbcSchemaUtils.drop(tables = allTables)
